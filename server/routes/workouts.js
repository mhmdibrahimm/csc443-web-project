...
// Every workout endpoint requires a logged-in user; ownership is then enforced
// per-row inside each handler.
...
router.get("/:id", async (req, res, next) => {
  try {
    ...
    // Join the entries with exercises so the response includes the exercise
    // name without forcing the client to issue a second request.
...
/**
 * POST /api/workouts
 * Body: { title, date, focus, durationMinutes, intensity, notes?, entries: [{ exerciseId, sets?, reps?, load?, durationMinutes? }] }
 * Inserts the workout + its entries inside a transaction.
 */
